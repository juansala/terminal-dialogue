...
void DrawHLine(int x_i, int y_i, int x_f, int y_f) {
  ...
  for (int x = x_i; x <= x_f; x++) {
    ...
    AddPixelChar(x, y, 'X', ncurses_wrapper::default_color_pairs::kClassic);
    if (decision_val > 0) {
      y = y + y_inc;
...
void DrawLine(int x_i, int y_i, int x_f, int y_f) {
  if (abs(y_f - y_i) < abs(x_f - x_i)) {
    if (x_i > x_f)
      DrawHLine(x_f, y_f, x_i, y_i);
    else
      DrawHLine(x_i, y_i, x_f, y_f);
  }
  else {
    if (y_i > y_f)
      DrawVLine(x_f, y_f, x_i, y_i);
    else
      DrawVLine(x_i, y_i, x_f, y_f);
  }
}